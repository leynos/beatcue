...
post-v1 extensions. ADR 008 records the object-tracking boundary for later
...
The command runs Hecate against the configured BeatCue policy and prints:

```text
hecate: architecture check passed
```

This development check does not change the planned video-analysis library API
or user-facing CLI contract.
...
JSON and WebVTT are v1 outputs. OTIO is planned as post-v1 editorial enrichment.
...
`msgspec.Struct`, as recorded in [ADR 007](adr-007-v1-beatcue-json-schema.md).
This is a contract for the future writer implementation; the current repository
does not yet provide the serializer.
...
work once the planned job commands are implemented:
...
schemes once delivery adapters are implemented.
